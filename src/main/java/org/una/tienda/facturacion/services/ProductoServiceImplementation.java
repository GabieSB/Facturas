...
public class ProductoServiceImplementation implements IProductoService {
    ...
    @Override
    @Transactional
    public Optional<ProductoDTO> update(ProductoDTO usuarioDTO, Long id) throws EModificarContenidoInactivoExeption {
        if (productoRepository.findById(id).isPresent()) {
            if(!usuarioDTO.isEstado()) throw new EModificarContenidoInactivoExeption("No se puede modificar un producto inactivo");
